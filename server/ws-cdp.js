...
function initCdpWs(server, db) {
    ...
    function handleSessionStart(ws, ctx, msg) {
        ...
        if (!tabId || typeof tabId !== 'number') {
            return wsSend(ws, { type: 'cdp:error', error: 'tabId (number) is required', code: 'INVALID_PARAMS' });
        }
    ...
    function handleCdpCommand(ws, ctx, msg) {
        ...
        if (!commandId || !method || !tabId) {
            return wsSend(ws, { type: 'cdp:error', error: 'commandId, method, and tabId are required', code: 'INVALID_PARAMS' });
        }
        ...
        try {
            ext.send(JSON.stringify({
                ...
                _auditId: auditId, // internal: extension echoes back for audit tracking
            }));
        } catch {
    ...
    function handleCdpResult(ws, ctx, msg) {
        const { commandId, result, error, durationMs, _auditId, tabId } = msg;
        if (!commandId) return;

        // Update audit log
        if (_auditId) {
            try {
                db.updateCdpAuditLog(_auditId, {
