...
3. **Agent Embed — 产品本身是 Agent** — ClawMark 不只是被动的消息管道，它本身具备 agent 能力：
   ...
   - 插件端：智能推荐标签、自动识别 bug vs feature request
   - 本质：消息管道 + 内置 AI Agent
...
| # | 问题 | 决策 |
|---|------|------|
| 1 | 消息归属 | 方案 C：URL pattern 自动匹配 + 手动选择 + 记忆学习 |
| 2 | 认证方式 | MVP 先用邀请码，OAuth/SSO 排后续 |
| 3 | 高亮持久化 | MVP 不做，只保存文本内容。后续按需投入 |
| 4 | 实时同步 | 目标是 WebSocket/SSE 实时推送，但排在后面 phase。MVP 先轮询 |
| 5 | 浏览器兼容 | 先做 Chrome，其他浏览器后续 |

## 待确认问题

1. **离线支持** — 离线时本地排队，上线后同步？
2. **Agent 能力边界** — 内置 AI Agent 第一版做到什么程度？（自动分类？智能分配？回复建议？）
