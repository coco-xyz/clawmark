...
- **环境**：
  ...
  - 生产环境：https://api.coco.xyz/clawmark（v0.3.0 已部署）

## 下一步：v0.3.5 — 用户自助与团队管理

**目标**：让用户自主注册、创建应用、管理端点，建立组织/团队体系。核心是替代 v0.3 的邀请码机制（不可 scale），实现用户自助 onboarding。

**用户自助体系：**
- **用户认证**：Google OAuth 登录/注册（替代邀请码）
- **应用管理**：用户自主创建 App，获得 AppKey（取代管理员手动分发）
- **投递端点管理**：用户登录后管理自己的投递 endpoint，设置默认 endpoint
...
**目标**：让项目主动声明路由偏好，标注自动路由，用户无需手动配置规则。增强扩展控制能力。

- **扩展 JS 注入控制**：用户可在扩展中选择停止/启用 JS 注入（按站点或全局），适用于不希望被标注功能影响的页面
