...
- [方式三：嵌入式 Widget](#方式三嵌入式-widget) ⏳ COMING SOON
...
```bash
# 将 YOUR_SERVER 替换为实际地址，如 https://labs.coco.xyz/clawmark
curl -X POST YOUR_SERVER/api/v2/items \
...
> **⏳ COMING SOON — 此功能尚未实现，以下为规划中的接口设计。**

ClawMark 计划提供可嵌入任意网站的 JS Widget，让访客无需安装扩展也能直接在页面上提交反馈。

```html
<!-- 规划中，尚未发布 -->
<script src="https://YOUR_SERVER/clawmark/widget.js"></script>
<script>
  ClawMark.init({ server: 'https://YOUR_SERVER/clawmark', apiKey: 'xxx' });
</script>
```
...
| **Base URL** | 默认 `https://gitlab.com`，自建实例填实际地址（如 `https://git.example.com`） |
| **Labels** | 可选 |
...
将反馈通过邮件发送给指定收件人。通过 HTTP API 发送（非 SMTP），支持 Resend 和 SendGrid 两种服务商。
...
> **获取方式：**
> - API Key：Linear → Settings → API → Create Key
> - Team ID：Linear → Settings → Teams → 选择团队 → URL 中的 UUID
> - Assignee ID：Linear → Settings → Members → 点击成员 → URL 中的 UUID
...
1. **Target Declaration** — 页面通过 `<meta name="clawmark-target" ...>` 标签声明的目标（已实现，详见 `server/target-declaration.js`）
