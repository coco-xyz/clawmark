...
**Author:** Jessie
**Date:** 2026-03-04
**Status:** Draft v8 — 已整合 Kevin review 反馈（#152-#157 + delivery 智能化 + 总开关 + popup 保留分发规则 + 分发目标置顶 + 标注窗口优化 + 覆盖层可拖拽可伸展）
...
Kevin 反馈：把标注窗口也拿出来优化一版。

#### 当前标注流程

1. 选中文字 → 浮动工具栏出现（4 按钮：Comment / Issue / Screenshot / Panel）
2. 点击 Comment 或 Issue → 输入覆盖层弹出
3. 输入覆盖层内容（从上到下）：标题 + 关闭按钮 / 引用文字 / 图片区 / 文本框 / 拖拽提示 / Dispatch 预览（默认隐藏）/ 标签（bug/feature/question）/ Submit

**问题：**
- Dispatch 目标默认隐藏，用户不知道标注会发到哪里
- 标签固定为 bug/feature/question，不可自定义
- 没有 delivery 目标选择 — 用户无法在标注时选择发到哪里
- 工具栏位置有时遮挡选中文字（v0.6.1 已部分修复）
...
变化不大，工具栏本身已经够简洁。增加一个溢出菜单（⋮）放次要操作。
...
│  " 选中的引用文字显示在这里... "       │  ← 引用预览
├───────────────────────────────────────┤
│  📮 发送到: ★ Slack #dev-team  [▾]   │  ← delivery 目标，默认显示推荐
├───────────────────────────────────────┤
│                                       │
│  Write your comment...                │  ← 文本输入框
│                                       │
│  📎 拖拽或粘贴图片                     │
├───────────────────────────────────────┤
│  [bug] [feature] [question] [+ 自定义]│  ← 标签，支持自定义
│                          [Submit ▶]   │
└───────────────────────────────────────┘
```

#### 覆盖层位置与交互优化（Kevin 反馈：位置很迷，遮住提交按钮）

当前问题：输入覆盖层出现的位置不可控，经常遮住页面内容或自身的提交按钮。

**解决方案：可拖拽 + 可伸展**

| 能力 | 实现 |
|------|------|
| **可拖拽** | 覆盖层标题栏（header）作为拖拽手柄，鼠标按住可移动整个覆盖层。`cursor: grab` 提示 |
| **可伸展** | 覆盖层右下角 resize 手柄，可调整宽高。文本框随容器自动伸缩（`resize: none` 改为容器控制） |
| **智能定位** | 默认出现在选区下方；如果下方空间不足，改为上方；如果左右超出视口，自动偏移。避免遮挡选中文字 |
| **位置记忆** | 记住用户上次拖拽/调整后的位置和尺寸（localStorage），下次打开时恢复 |
| **最小尺寸** | 宽度 min 300px，高度 min 200px，防止缩太小不可用 |
| **边界约束** | 拖拽不能超出视口边界（clamp to viewport） |

#### 关键改进

| 项目 | 当前 | 优化后 |
|------|------|--------|
| 覆盖层位置 | 固定出现在选区附近，不可移动 | **可拖拽**，标题栏为拖拽手柄，位置记忆 |
| 覆盖层大小 | 固定尺寸 | **可伸展**，右下角 resize 手柄，尺寸记忆 |
| 智能定位 | 有时遮住选区或提交按钮 | 自动避让选区 + 视口约束 |
| Delivery 目标 | 隐藏在底部，大多数用户看不到 | **输入框上方显眼位置**，默认展示推荐目标 |
| 目标选择 | 无法选择 | 下拉选择所有已配置目标，推荐排第一 |
| 标签 | 固定 3 个（bug/feature/question） | 支持自定义标签 |
| 图片 | 支持粘贴/拖拽 | 保持不变，优化提示文案 |
| 快捷键 | 无 | Cmd+Enter 提交 |
| 提交反馈 | 按钮变灰 + "..." | 进度条 + 成功/失败 toast |
...
*PRD by Jessie, 2026-03-04 (v8). Based on issue #150 (Kevin) + all inline review feedback + ClawMark extension v0.6.1 代码分析。*
