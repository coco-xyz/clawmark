...
**Author:** Jessie
**Date:** 2026-03-04
**Status:** Draft v6 — 已整合 Kevin review 反馈（#152-#157 + delivery 智能化 + 总开关 + popup 保留分发规则 + 分发目标置顶）
...
- **Popup = 当前页面的操作面板**，放当前页面相关内容 + 常用功能（含分发规则快捷操作）
- **Options Page（Dashboard）= 完整管理中心**，独立全屏页面，放高级管理（批量、导入导出、测试）
...
```
┌─────────────────────────────┐
│  ClawMark    [🔘 ON]  ⚙️    │  ← 总开关（全局启停）+ 齿轮→Dashboard
├─────────────────────────────┤
│  📄 github.com/coco-xyz/... │  ← 当前 tab
│  ───────────────────────────│
│  📮 分发到:                   │  ← 最重要：标注会发到哪里
│  ★ Slack #dev-team (推荐)    │  ← 智能推荐（基于 URL+历史）
│  · GitHub Issue              │  ← 其他可选目标
│  · [更多目标 ▾]  [+ 新增]    │  ← 展开候选列表 / 快捷新增
│  ───────────────────────────│
│  💬 3 Comments  🐛 1 Issue   │  ← 当前页面标注统计（可点击）
│  · "这段代码有 bug" — 2m ago │  ← 最近标注预览
...
**总开关（Master Toggle）说明**（Kevin 反馈：总开关是很常用的功能）：
- 位于 popup header 最显眼位置，紧挨 ClawMark 标题
- 一键全局启停 ClawMark（关闭后所有页面停止注入标注功能）
- 关闭状态下 popup 内容变灰，底部显示「ClawMark 已关闭」
- 与站点开关的关系：总开关 > 站点开关。总开关关闭时，站点开关不可操作
- 场景：临时不想标注（开会、分享屏幕、调试其他插件），一键关闭比逐站禁用方便得多
...
| Delivery Rules | popup 里折叠面板 | popup 保留快捷分发（推荐+选择+候选），Dashboard 放完整管理（导入导出、测试、批量） |
| 登录状态 | popup 大块区域 | 齿轮 → Dashboard 里管理 |
| 当前页面标注数 | 无 | popup 顶部显示 |
| 最近标注 | 无 | popup 中间区域 |
| 站点开关 | popup 里 | 保留，精简为一行 |
| 总开关（Master Toggle） | 无 | popup header 显眼位置，一键全局启停 |
...
**📮 分发规则**（Kevin 反馈：规则应在 popup 里处理）

Popup 和 Dashboard **分层处理**分发规则：

**Popup 里（日常操作）：**
- 智能推荐当前页面的最佳分发目标（基于 URL pattern + 历史 + 标注类型）
- 可选择其他已配置的分发目标
- 候选列表按匹配度排序
- 快捷新增分发目标（简化表单：名称 + URL）
- 标注时直接选目标→发送，不用离开 popup

**Dashboard 里（高级管理）：**
- 规则列表表格（可排序、可搜索）
- 完整的新增/编辑规则表单（URL pattern、匹配条件、优先级）
- 规则测试：输入一个 URL 看会匹配到哪些规则
- 导入/导出规则（JSON）
- 测试连通性
- 预置规则模板（如「GitHub PR → GitHub Issue」「Notion → Slack」）
...
- 将 Server URL、Username、Auth 从 popup 迁移到 Dashboard
- 分发规则：popup 保留快捷操作（推荐+选择+新增），Dashboard 新增完整管理（批量、导入导出、测试）
- Popup 改版为当前页面视角（标注统计 + 最近标注 + 分发目标 + 总开关 + 快捷操作）
- 齿轮图标打开 Dashboard
...
*PRD by Jessie, 2026-03-04 (v6). Based on issue #150 (Kevin) + Kevin review #152-#157 + delivery 智能化 + 总开关 + popup 保留分发规则 + 分发目标置顶 + ClawMark extension v0.6.1 代码分析。*
