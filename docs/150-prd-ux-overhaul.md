...
**Author:** Jessie
**Date:** 2026-03-04
**Status:** Draft — 待 Kevin 确认后 Boot 实现
...
- **首次使用零配置**，默认连接 api.coco.xyz，Google 一键登录即可开始
...
- 当前登录状态 + 头像
- Google 登录 / 登出
- 手动认证（API Key / Invite Code）
- Username 设置

**🔗 连接**
- Server URL 配置（默认 api.coco.xyz，不推荐修改）
- 连接状态 + 延迟测试
- 版本信息
...
- Server URL 默认 `api.coco.xyz`，隐藏在高级设置里
...
2. **是否保留 Manual Auth（API Key / Invite Code）？** 如果目标是傻瓜化，是否只保留 Google 登录？
3. **规则模板**：是否预置常用规则模板（如「所有标注发 GitHub Issue」）？

---

*PRD by Jessie, 2026-03-04. Based on issue #150 (Kevin) + ClawMark extension v0.6.1 代码分析。*
